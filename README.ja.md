...
- **一部は計測済みです。** セッション内では、compaction によって実コンテキスト
  （1リクエストあたり input+cache_read+cache_creation）が実測29件で**中央値66%**
  縮み、縮小率は圧縮前サイズに単調依存します（Spearman ρ=0.977）。圧縮後フロアは
  約50〜64kトークンなので、フロアの約2倍未満での早期 `/compact` は得が小さいです。
  [`scripts/compaction_effect.py`](scripts/compaction_effect.py) で再現できます。
  日跨ぎでは、ワークロード正規化した4日系列
  （[docs/verification-2026-07-05.md](docs/verification-2026-07-05.md)）で、
  セッション中央値の cacheRead/output 比が閉ループ稼働後に
  **233x → 130x → 110x → 83x と単調改善**しました。セッションはむしろ長時間化
  （req 中央値 2→59）しており、通常この比を悪化させる方向なので、compact 効果と
  整合します。一方、cache read 比率（約94%）と hot セッション率は**改善して
  いません**。n=4日・1ユーザーのため、因果の証明ではなく整合性の根拠です。

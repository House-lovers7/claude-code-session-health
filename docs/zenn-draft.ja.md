...
このうち、**セッション内の削減効果は翌日の実測で確認できた**ので、次の追記に載せる。日をまたいだ集計コストの傾向は、引き続き継続観測中である。
...
* closed-loop の作動確認に加えて、**/compact がセッション内の live context を中央値66%縮める**ことまで実測できた。日をまたいだコスト傾向は継続観測中

...
**この仕組みによる定量的な削減効果は未検証です。** フックは発火し、モデルは
実際に /compact や委譲を提案し始めていますが、「cacheRead/output 比が
何倍から何倍に下がったか」の before/after データはまだありません。
本記事は第1報（実測手法と closed-loop の仕組みの共有）で、数日運用した
改善データは第2報として出します。
